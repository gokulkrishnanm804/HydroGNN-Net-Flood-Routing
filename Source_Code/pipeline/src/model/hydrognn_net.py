...
class HydroGNNNet(nn.Module):
    ...
    def __init__(
        ...
        # 6. Trend-conditioned persistence gating head (Experiment 9)
        gate_in_dim = head_in_dim + 3
        self.gating_head       = nn.Linear(gate_in_dim, len(horizons))
    ...
    def forward(
        ...
        trend        : [N, 3]     observable historical trends (6h, 12h, 24h)
        return_attention: Not yet used (reserved for GNN-XAI).
        return_gate  : If True, returns (gated_delta, log_var, gate).

        Returns
        -------
        If return_gate is False:
            (gated_delta, log_var)
        If return_gate is True:
            (gated_delta, log_var, gate)
        """
        # 1. Temporal encoding: [N, T, F] → [N, hidden]
        h_t = self.temporal_encoder(x)

        # 2. Spatial encoding via GATv2: [N, hidden] → [N, gat_out]
        ...
        # 5. Prediction heads
        raw_gnn_delta = self.forecast_head(h_cond)      # [N, H]
        log_var       = self.uncertainty_head(h_cond)   # [N, H]

        # 6. Trend-conditioned persistence gating head (Experiment 9)
        if trend is None:
            trend_feat = x.new_zeros((N, 3))
        else:
            trend_feat = trend.unsqueeze(-1) if trend.ndim == 1 else trend

        gate_input    = torch.cat([h_cond, trend_feat], dim=-1) # [N, head_in_dim + 3]
        gate_logits   = self.gating_head(gate_input)            # [N, H]
        gate          = torch.sigmoid(gate_logits)              # [N, H], values in [0, 1]

        # 7. Gated delta prediction
        gated_delta   = gate * raw_gnn_delta                    # [N, H]

        if return_gate:
            return gated_delta, log_var, gate

        return gated_delta, log_var
...
class HydroGNNLoss(nn.Module):
    ...
    L = w_mse      x MSE(pred, target)
      + w_mae      x SmoothL1(pred, target)
      + w_nse      x (1 - NSE(pred, target))
      + w_phys     x continuity_violation
      + w_dreg     x delta_reg          [Experiment 8: persistence-residual gated]
    ...
    Regime-aware Delta Regularization (Experiment 8):
        Three regimes based on |actual_delta| = |y(t+h) - y(t)|:
        - Nearly stationary (|delta| < 0.01 m):
            Zero-push: MSE(pred, 0)
        - Small motion (0.01 <= |delta| < 0.10 m):
            Directional pull: 0.25 * MSE(pred, target)
        - Dynamic (|delta| >= 0.10 m):
            No delta regularization (standard MSE/SmoothL1 handles dynamic routing)

        Combined: loss_dreg = loss_stat + 0.25 * loss_small
    ...
    delta_reg_weight: Weight for regime-aware delta regularization (default 0.25).
    """
    ...
    def forward(
        self,
        pred:    Tensor,                   # [N, H] (gated_delta)
        target:  Tensor,                   # [N, H] (delta_y)
        log_var: Optional[Tensor] = None,  # [N, H]
        mask:    Optional[Tensor] = None,  # [N] or [N, H] bool
    ) -> Tensor:
        """
        Compute total loss.
        ...
        Parameters
        ----------
        pred    : Predicted gated delta [N, H].
        target  : True delta [N, H].
        log_var : Log-variance for NLL term [N, H] (optional).
        mask    : Boolean mask — True where target is valid [N] or [N, H] (optional).
        """
        ...
        # 6. Regime-aware delta regularization
        # target is delta_y = y(t+h) - y(t). Three regimes:
        #   nearly stationary : |delta| < 0.01 m  -> strong zero-push
        #   small motion      : 0.01 <= |delta| < 0.10 m -> weak directional pull
        #   dynamic           : |delta| >= 0.10 m -> no delta regularization
        nearly_stationary = valid & (target.abs() < 0.01)
        small_motion      = valid & (target.abs() >= 0.01) & (target.abs() < 0.10)

        if nearly_stationary.any():
            loss_stat = (pred[nearly_stationary] ** 2).mean()
        else:
            loss_stat = pred.new_zeros(1).squeeze()

        if small_motion.any():
            p_sm = pred[small_motion]
            t_sm = target[small_motion]
            loss_small = ((p_sm - t_sm) ** 2).mean()
        else:
            loss_small = pred.new_zeros(1).squeeze()

        loss_dreg = loss_stat + 0.25 * loss_small
